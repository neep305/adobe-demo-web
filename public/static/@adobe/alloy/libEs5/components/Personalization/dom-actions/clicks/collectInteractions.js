...
const getInteractionDetail = clickedElement => {
  ...
  while (element && element !== documentElement) {
    const interactId = (0, _getAttribute.default)(element, _createDecorateProposition.INTERACT_ID_DATA_ATTRIBUTE);
    if (interactId) {
...
const extractViewName = metas => {
  const foundMetaWithScopeTypeView = metas.find(meta => meta.scopeType === _scopeType.VIEW_SCOPE_TYPE);
...
var _default = (clickedElement, getInteractionMetas, autoCollectPropositionInteractions) => {
  const {
    interactIds,
    clickLabel = "",
    clickToken
