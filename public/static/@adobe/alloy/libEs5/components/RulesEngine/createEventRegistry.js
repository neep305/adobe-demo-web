...
const createEventPruner = (limit = MAX_EVENT_RECORDS, retentionPeriod = RETENTION_PERIOD) => {
  return events => {
    ...
    Object.keys(events).forEach(eventType => {
      pruned[eventType] = {};
      Object.values(events[eventType]).filter(entry => new Date(entry.firstTimestamp) >= (0, _utils.getExpirationDate)(retentionPeriod)).sort((a, b) => a.firstTimestamp - b.firstTimestamp).slice(-1 * limit).forEach(entry => {
...
var _default = ({
  ...
  const addExperienceEdgeEvent = event => {
    ...
    const validPropositionEventType = propositionEventType => propositionEventTypeObj[propositionEventType] === _eventType.EVENT_TYPE_TRUE;
    const {
      id: action
    } = propositionAction;
